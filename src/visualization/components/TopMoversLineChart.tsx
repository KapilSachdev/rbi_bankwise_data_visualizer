...
const TopMoversLineChart: FC<TopMoversLineChartProps> = ({ allData, months, metric = 'total', topN = 5, chartRef }) => {
  ...
  // period hook (exposed by the PeriodPresets filter for modularity)
  const {
    selectedPreset,
    setSelectedPreset,
    monthRange,
  ...
  const bankSeries = useMemo(() => {
    ...
    filteredMonths.forEach((month, mi) => {
      ...
      monthData.forEach(row => {
        ...
        const value = selectedMetric === 'credit'
          ? (credit?.at_PoS?.Volume || 0) + (credit?.Online_ecom?.Volume || 0)
  ...
  const topBanks = useMemo(() =>
    [...bankSeries]
      ...
      .slice(0, topNState), [bankSeries, topNState]);
      ...
      <div className="grid gap-2">
        ...
        <div className="flex w-full items-center justify-between gap-6 mb-2">
          ...
          <TriangleSwitch
            ...
            onSelect={option => setSelectedMetric(option === 'Total' ? 'total' : option === 'Credit' ? 'credit' : 'debit')}
            size={48}
          />
        </div>
